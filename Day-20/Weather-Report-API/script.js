...
container.innerHTML = `      
    ...
    <div class="card-body">
        <span class="card-title" id="location"></span><br>
        <span class="card-title" id="weather"></span><br>
        <span class="card-title" id="temperature"></span>
     </div>`;
...
async function area() {
  ...
  let lat_long = await fetch(
    `https://api.openweathermap.org/geo/1.0/direct?q=${location}&appid=3226ce0ee6d73b13a0cb40a0fb71540e`
  );
  let res = await lat_long.json();
  //fetch weather data using weather api
  let weather_data = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${res[0].lat}&lon=${res[0].lon}&appid=3226ce0ee6d73b13a0cb40a0fb71540e`
  );
